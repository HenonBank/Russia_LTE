...
# ==========================================
# 🚀 ЗАПУСК ОБЪЕДИНЕННОГО БОТА (ИСПРАВЛЕННЫЙ)
# ==========================================
async def main():
    ...
    # Регистрация хендлеров Ads Bot
    ads_dp.message.register(cmd_start, Command("start"))
    ...
    # 5. ЗАПУСК ОБОИХ БОТОВ ПРАВИЛЬНО
    try:
        # Запускаем оба бота в параллельных задачах
        tasks = [
            asyncio.create_task(ads_dp.start_polling(ads_bot)),
            asyncio.create_task(glass_app.run_polling())
        ]
        
        logging.info("✅ Оба бота успешно запущены и работают параллельно!")
